...
(function() {
    ...
    Runner.prototype = {
        ...
        loadSounds: function() {
            if (!IS_IOS && AudioContext) { // Fix 12.06.17
                ...
                // Skip sound loading if template doesn't exist
                if (!resourceTemplate) {
                    return;
                }
                this.audioContext = new AudioContext();
                var content = resourceTemplate.content;
                for (var sound in Runner.sounds) {
                    var soundEl = content.getElementById(Runner.sounds[sound]);
                    if (soundEl) {
    ...
    // Expose Runner globally for manual initialization
    window.Runner = Runner;
})();
// Removed auto-start - will be initialized manually when Stage 4 loads
